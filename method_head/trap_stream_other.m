function [avgF1_k_off1, avgAUC_k_off1, OA_k_off1, AA_k_off1, times_k_off1] = trap_stream_other(data_all, labels_all, train_ratio, k)
data = data_all;
labels = labels_all;
...
for i=1:1
    ...
    %STSD         
    [classifier_stsd, ~, timeUse, ~, ~, ~, ~] = STSD(X, Y, NumFeature, ID, 0.1 , lambda, 0);
end
