...
function info = muteinf(A, Y)
    ...
    % 确保输入矩阵的大小匹配
    assert(size(A, 1) == size(Y, 1), '输入矩阵的行数不匹配');
    ...
    % 初始化互信息矩阵
    info = zeros(num_features, num_labels);

    % 计算每个特征和标签之间的互信息
    for i = 1:num_features
        for j = 1:num_labels
            % 计算特征和标签的联合概率分布
            P_xy = histcounts2(A(:, i), Y(:, j));
            ...
            % 计算互信息
            P_xy = P_xy / sum(P_xy(:));  % 归一化联合概率分布
            P_x = P_x / sum(P_x);        % 归一化边缘概率分布
            P_y = P_y / sum(P_y);        % 归一化边缘概率分布

            info(i, j) = sum(sum(P_xy .* log2(P_xy ./ (P_x * P_y))));
        end
    end
end
%  =========================================================================
